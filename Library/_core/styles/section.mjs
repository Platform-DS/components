...
// and one stylesheet. Unlayered, so ordinary specificity decides: these rules
// beat a page's bare `p`/`h2`/`ul` styles, and a consumer's class or id still
// beats these. See injectStyles for the measurements behind that.
...
-------------------------------------------------*/
/* NOT :where(), unlike the typography below: these have to outrank a
   page's bare p { margin: 3rem }, which is the exact bleed that
   unlayering these sheets exists to stop. */
${SEL} > * { margin-block: 0; }
${SEL} > * + * { margin-block-start: var(--pl-size-16, 1rem); }

:where(${SEL}) :is(h1, h2, h3, h4) {
