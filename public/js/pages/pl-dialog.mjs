...
export default () => page(
    ...
    demo(`
        ...
        <pl-dialog id="demo-confirm" dismissable closedby="any">
            ...
            <div data-actions>
                ...
                <form method="dialog"><pl-button variant="danger" value="delete">Delete</pl-button></form>
            </div>
        </pl-dialog>
    `, { layout: 'stack' }),
    ...
    table(
        ['Mechanism', 'How'],
        [
            { cells: ['<code>&lt;form method="dialog"&gt;</code>', 'A submit button inside the form closes the dialog and reports its <code>value</code> as <code>returnValue</code>. Needs no id.'] },
    ...
    code(`
        ...
        document.querySelector('pl-dialog')
            .addEventListener('pl-close', event => {
                console.log(event.detail.returnValue);   // the submit button's value
            });
    `, 'js'),
